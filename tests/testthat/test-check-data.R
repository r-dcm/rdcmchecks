test_that("check data", {
  ...
  test_d <- data.frame(resp = paste0("I", 1:5),
  ...
  test_d <- data.frame(resp = sample(0:1, 5, replace = TRUE),
  ...
  test1_d <- tibble::tibble(resp = paste0("I_", 1:5),
                            item1 = c(0, 1, 1, 0, 1),
                            item2 = c(1, 0, 0, 1, 0),
                            item3 = c(1, 1, 1, 0, 0))
  test2_d <- tibble::tibble(resp = paste0("I_", 1:5),
                            item1 = c(0L, 1L, 1L, 0L, 1L),
                            item2 = c(1L, 0L, 0L, 1L, 0L),
                            item3 = c(1L, 1L, 1L, 0L, 0L))
  test3_d <- tibble::tibble(item1 = c(0L, 1L, 1L, 0L, 1L),
                            item2 = c(1L, 0L, 0L, 1L, 0L),
                            item3 = c(1L, 1L, 1L, 0L, 0L))
  test4_d <- data.frame(resp = paste0("I_", 1:5),
                        item1 = c(0, 1, 1, 0, 1),
                        item2 = c(1, 0, 0, 1, 0),
                        item3 = c(1, 1, 1, 0, 0))
  test5_d <- data.frame(resp = paste0("I_", 1:5),
                        item1 = c(0L, 1L, 1L, 0L, 1L),
                        item2 = c(1L, 0L, 0L, 1L, 0L),
                        item3 = c(1L, 1L, 1L, 0L, 0L))
  test6_d <- data.frame(item1 = c(0L, 1L, 1L, 0L, 1L),
                        item2 = c(1L, 0L, 0L, 1L, 0L),
                        item3 = c(1L, 1L, 1L, 0L, 0L))
  check_d <- tibble::tibble(resp = paste0("I_", 1:5),
                            item1 = c(0L, 1L, 1L, 0L, 1L),
                            item2 = c(1L, 0L, 0L, 1L, 0L),
                            item3 = c(1L, 1L, 1L, 0L, 0L))
  check_d_null <- tibble::tibble(item1 = c(0L, 1L, 1L, 0L, 1L),
                                 item2 = c(1L, 0L, 0L, 1L, 0L),
                                 item3 = c(1L, 1L, 1L, 0L, 0L))
...
test_that("clean data", {
  ...
  cleaned_d <- clean_data(dcmdata::dtmr_data, identifier = "id",
                          cleaned_qmatrix = cleaned_q)

  expect_equal(length(cleaned_d), 5)
  expect_equal(names(cleaned_d), c("clean_data", "item_identifier",
                                   "item_names", "respondent_identifier",
                                   "respondent_names"))

  expect_equal(ncol(cleaned_d$clean_data), 3)
  expect_equal(nrow(cleaned_d$clean_data),
               nrow(dcmdata::dtmr_data) * (ncol(dcmdata::dtmr_data) - 1))
  expect_equal(colnames(cleaned_d$clean_data), c("resp_id", "item_id", "score"))
  expect_equal(vapply(cleaned_d$clean_data, typeof, character(1)),
               c(resp_id = "integer", item_id = "integer", score = "integer"))
  expect_equal(vapply(cleaned_d$clean_data, is.factor, logical(1)),
  ...
  cleaned_d <- clean_data(dcmdata::mdm_data, identifier = "respondent",
                          cleaned_qmatrix = cleaned_q)
...
test_that("reconcile item names", {
  ...
  err <- rlang::catch_cnd(
    clean_data(x = my_dat, identifier = "respondent",
               cleaned_qmatrix = cleaned_q)
  )
  expect_s3_class(err, "rlang_error")
  ...
  expect_match(err$footer[1], paste0("Items found in `my_dat` but not ",
                                     "`cleaned_q`:.*J1.* and .*J13.*"))
  expect_match(err$footer[2], paste0("Items found in `cleaned_q` but not ",
                                     "`my_dat`:.*E1.* and .*E13.*"))
})
...
test_that("clean new data", {
  ...
  err <- rlang::catch_cnd(
    clean_data(new_data, identifier = "id", cleaned_qmatrix = cleaned_q,
  ...
  cleaned_d <- clean_data(new_data, identifier = "resp_id",
                          cleaned_qmatrix = cleaned_q,
                          valid_names = cleaned_q$item_names)
  ...
  ## errors with new items -----------------------------------------------------
  cleaned_q <- clean_qmatrix(dcmdata::mdm_qmatrix[, -1])
  orig_data <- clean_data(dcmdata::mdm_data, identifier = "respondent",
                          cleaned_qmatrix = cleaned_q)
  new_data <- dplyr::mutate(dcmdata::mdm_data, mdm5 = 0)

  err <- rlang::catch_cnd(
    clean_data(new_data, identifier = "respondent", cleaned_qmatrix = cleaned_q,
  ...
  orig_data <- clean_data(dcmdata::dtmr_data, identifier = "id",
                          cleaned_qmatrix = cleaned_q)
  new_data <- dcmdata::dtmr_data |>
    dplyr::select(1:7) |>
    dplyr::slice_sample(n = 10)
  cleaned_d <- clean_data(new_data, identifier = "id",
                          cleaned_qmatrix = cleaned_q,
                          valid_names = orig_data$item_names)
